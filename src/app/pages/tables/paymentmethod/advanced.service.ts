...
import { PaymentMethods, SearchResult } from '../../../core/models/payment_methods.models';
 ...
 */
function sort(tables: PaymentMethods[], column: string, direction: string): PaymentMethods[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: PaymentMethods, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<PaymentMethods[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: PaymentMethods[];
    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataPMService,) {
