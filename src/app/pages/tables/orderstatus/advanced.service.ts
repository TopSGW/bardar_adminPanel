...
import { OrderStatus, SearchResult  } from '../../../core/models/order_status.models';
 ...
 */
function sort(tables: OrderStatus[], column: string, direction: string): OrderStatus[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: OrderStatus, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<OrderStatus[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: OrderStatus[];
    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataOrderStatusrvice,) {
