...
import { SharedDataMarketCategoryService } from './data';
...
@Component({
  ...
  templateUrl: './marketcategories.component.html',
  styleUrls: ['./marketcategories.component.scss'],
...
export class MarketCategoriesComponent implements OnInit {
  ...
  // Table data
  tableData: MarketCategory[];
  public selected: any;
  hideme: boolean[] = [];
  tables$: Observable<MarketCategory[]>;
  ...
  constructor(public service: AdvancedService,
    ...
    private sharedDataService: SharedDataMarketCategoryService,
  ) {
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Market Setting' }, { label: 'Market Categories', active: true }];
    /**
     * fetch data
     */
    // this._fetchData();
    this.sharedDataService.currentTable.subscribe(tableData => (this.tableData = tableData));
    this.setserv.getMarketCategory().subscribe(val => this.sharedDataService.changeTable(val));
  ...
  deleteGroupItem(id) {
    this.setserv.delMarketCategory(id).subscribe(data => {
      let newTable = this.tableData.filter(data => data.id != id);
