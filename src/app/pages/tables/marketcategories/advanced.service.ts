...
import { MarketCategory, SearchResult } from '../../../core/models/market_category.models';
 ...
 */
function sort(tables: MarketCategory[], column: string, direction: string): MarketCategory[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: MarketCategory, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<MarketCategory[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: MarketCategory[];

    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataMarketCategoryService,) {
