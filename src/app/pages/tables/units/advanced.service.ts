...
import { Units, SearchResult } from '../../../core/models/units.models';
 ...
 */
function sort(tables: Units[], column: string, direction: string): Units[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: Units, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Units[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: Units[];

    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataUnitsService,) {
