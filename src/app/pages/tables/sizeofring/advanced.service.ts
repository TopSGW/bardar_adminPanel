...
import { Sizes, SearchResult } from '../../../core/models/sizes.models';
 ...
 */
function sort(tables: Sizes[], column: string, direction: string): Sizes[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: Sizes, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Sizes[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: Sizes[];

    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataSizeService,) {
